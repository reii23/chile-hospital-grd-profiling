...
\begin{table}[htbp]
\centering
\caption{Contribución de los macro-bloques a la distancia euclidiana cuadrada en la matriz preprocesada.}
...
\begin{table}[htbp]
\centering
\caption{Sensibilidad de Ward $K=4$ a la ponderación por bloques y a la exclusión del bloque demográfico-obstétrico.}
...
\caption{Dendrograma Ward del Nivel~1, partición en cuatro clústeres ($K=4$).}
...
\caption{Proyecciones PCA de la matriz institucional para las soluciones Ward con $K=2$, $K=3$ y $K=4$.}
...
\begin{table}[htbp]
\centering
\caption{Nulas del flujo Ward $K=4$ y Kruskal--Wallis/BH, variables significativas de 28 evaluadas.}
...
\begin{table}[htbp]
\centering
\caption{Comparación de homogeneidad interna controlada por número de grupos entre Ward $K=2$ y MINSAL.}
...
\begin{table}[htbp]
\centering
\caption{Validación cruzada de homogeneidad interna por bloques de variables (Ward $K=4$ y $K=2$ vs.~MINSAL).}
...
\begin{minipage}{\textwidth}
\normalsize\raggedright
\textit{Nota.} Ward se ajusta solo en el bloque de agrupamiento, ambas particiones se evalúan en el bloque retenido. La diferencia reportada es $\mathrm{WSS/TSS}_{\mathrm{MINSAL}}-\mathrm{WSS/TSS}_{\mathrm{Ward}}$; valores positivos favorecen menor dispersión para Ward. Los intervalos se obtuvieron mediante 5.000 remuestreos de los
