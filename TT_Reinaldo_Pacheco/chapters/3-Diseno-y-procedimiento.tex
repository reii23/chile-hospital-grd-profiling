...
\texttt{requirements.txt} declaran las versiones exactas de las dependencias de
ejecución, mientras que \texttt{requirements-dev.txt} incorpora las herramientas
de desarrollo y pruebas junto con esas dependencias. El archivo
\texttt{pyproject.toml} consolida los metadatos del paquete y la configuración
de \texttt{pytest}. Las semillas aleatorias están fijadas en el código
(\texttt{RANDOM\_STATE = 42}), de modo que cualquier ejecución posterior
reproduzca los mismos resultados. La verificación del comportamiento de los
módulos se automatiza con \texttt{pytest} y \texttt{hypothesis} en el directorio
\texttt{tests/}.
...
matemáticamente óptima. Ambas dimensiones se complementan con un contraste descriptivo respecto de la
clasificación administrativa del MINSAL: se examinan la concordancia entre
particiones y la compactación de los grupos en bloques de variables no utilizados
para construir cada partición. Este contraste no se interpreta como una prueba de
superioridad de una clasificación sobre la otra, pues ambas responden a criterios
distintos y la granularidad de las particiones también influye en su dispersión
interna.
...
La distinción entre hospitalización convencional, hospitalización diurna y CMA
es relevante para el perfilamiento porque estas modalidades responden a lógicas
funcionales distintas. La CMA se asocia a procedimientos ambulatorios, mientras
que la actividad diurna requiere atención y eventualmente cama durante el día,
sin pernoctación; ambas difieren de los episodios de hospitalización nocturna en
la duración y el consumo de recursos. Por ello, los indicadores de estancia,
peso GRD, severidad, mortalidad y casuística hospitalaria se calculan solo sobre
hospitalización convencional y de urgencia, excluyendo tanto CMA como
hospitalización diurna. Esta última se conserva como modalidad analítica
separada y no se incorpora a la tasa de CMA. La distribución de los registros
por tipo de actividad se presenta en la Tabla~\ref{tab:actividad}.
...
La decisión de separar la hospitalización diurna se verificó empíricamente en
los 65 establecimientos elegibles. Aunque representa el 1{,}13\,% de los
episodios inicialmente agrupados como hospitalización y su mediana institucional
es 0{,}21\,\%, su distribución es heterogénea: cuatro establecimientos superan
5\,% y uno alcanza 12{,}92\,%. Al excluirla de los denominadores hospitalarios,
el cambio máximo en la estancia media es 14{,}83\,% y en el peso medio GRD es
6{,}71\,%. Por tanto, conservarla mezclada con hospitalización nocturna podría
alterar de forma diferencial el perfil de un subconjunto de establecimientos,
aun cuando su incidencia nacional sea reducida. El detalle reproducible por
establecimiento se reporta en el archivo
\texttt{hospitalizacion\_diurna\_por\_establecimiento.csv}.
...
del período, umbral que responde a que estimaciones como la estancia media, la
proporción de CMA o la diversidad diagnóstica requieren varios años de
observación para no depender de particularidades circunstanciales de un único
ejercicio.

La distribución real de años presentes entre los 72 establecimientos es
marcadamente bimodal (Tabla~\ref{tab:distribucion-anios}): 7 hospitales
reportan datos en uno o dos años (todos incorporados al sistema recién en
2023 o 2024) mientras que los 65 restantes tienen cobertura completa del
período. Ningún establecimiento tiene exactamente 3, 4 o 5 años de datos. En
consecuencia, cualquier umbral en ese rango produce el mismo universo final de
65 hospitales elegibles: la elección del valor ``3'' opera sobre un vacío
natural de los datos, sin consecuencias prácticas sobre qué establecimientos
se incluyen.
...
Para cuantificar la ganancia de precisión que justifica el umbral, se realizó
una simulación de error por submuestreo sobre los 65 hospitales con cobertura
completa. El error relativo mediano decrece de forma monótona pero con
rendimientos decrecientes: la mayor reducción se produce entre $k=1$ y $k=3$
años ($8{,}2\,\%$ a $4{,}1\,\%$ de error promedio en seis variables), mientras
que agregar un cuarto o quinto año reduce el error en apenas $1{,}4$ y
$1{,}2$ puntos adicionales. Tres años constituye así el primer punto en que la
reducción del error empieza a estabilizarse. Los resultados completos de la
simulación se presentan en la
Tabla~\ref{tab:simulacion-anios} del Anexo~\ref{anexo:hospitales}.
...
institucional descrita en la sección siguiente. El efecto sobre la partición
final se evalúa en la Sección~\ref{sec:sensibilidad-elegibilidad}. El detalle
de los siete establecimientos excluidos se presenta en la
Tabla~\ref{tab:anexo-excluidos} del Anexo~\ref{anexo:hospitales}.
...
\midrule
\endfirsthead
\multicolumn{2}{l}{\textit{Tabla~\thetable\ (continuación)}} \\
\toprule
\textbf{Grupo} & \textbf{Variables} \\
\midrule
\endhead
...
Distribución demográfica (\% pediátrico, \% geriátrico, \% femenino fértil, edad
mediana), mezcla por tipo de ingreso (\% urgencia, \% programado, \% obstétrica),
tipo de alta (\% domicilio, \% traslado, \% fallecido), procedencia (\% origen
emergencia, \% origen referencia), actividad quirúrgica (\% uso de pabellón,
promedio de usos de pabellón), actividad obstétrico-neonatal (proporción de
egresos con condición de alta neonatal, proporción de esos egresos con peso de
nacimiento inferior a 2.500 gramos) y estadía (\% estancia larga, coeficiente de
variación). \\
\bottomrule
\end{longtable}
}
...
alta. \\
\% origen emergencia / referencia & Proporción de egresos según procedencia:
servicio de urgencia propio o derivación con procedencia normalizada
\texttt{OTROS\_HOSPITALES} o \texttt{RED\_NACIONAL}. \\
...
\paragraph{Reducción dimensional de la casuística clínica}
\label{sec:pca}
...
capítulo o una sección de la tabla maestra, mientras que en el tercero agrupa los GRD que
...
En el pipeline se utilizó la variante ponderada porque incorpora
los diagnósticos secundarios disponibles para representar la carga clínica
registrada, manteniendo el diagnóstico principal con el doble de peso para que
el motivo de ingreso siga siendo dominante. La elección no se justifica por la
diferencia aislada de varianza acumulada: con $K=8$, la variante ponderada
retiene $72{,}3\,\%$ frente a $71{,}8\,\%$ de la variante principal, una
diferencia pequeña que se reporta solo como antecedente descriptivo.

La sensibilidad se evaluó reconstruyendo la matriz completa y aplicando Ward
con $K=4$ bajo ambas variantes. La concordancia fue alta (ARI $=0{,}936$;
NMI $=0{,}909$): los perfiles pediátrico y de especialización clínica se
mantuvieron, y solo el Hospital San José de Parral pasó del grupo de carga
clínica al núcleo generalista. La variante ponderada obtuvo un Silhouette de
$0{,}399$, frente a $0{,}375$ con la variante principal. Estos resultados
muestran que la decisión de incorporar diagnósticos secundarios tiene una
influencia acotada, pero no nula, sobre la frontera entre esos dos grupos. No
constituyen una prueba de superioridad, debido a que, ambas construcciones se evalúan en
los mismos hospitales y período. Las asignaciones, la contingencia y las
métricas de esta sensibilidad se conservan en los artefactos reproducibles del
repositorio. La misma comparación de varianza se repite bajo una representación
composicional alternativa (CLR) como parte del análisis definido en la
Sección~\ref{sec:sensibilidad-composicional}.
...
que acumularía el 80\,\% de la varianza) el coeficiente de Silhouette desciende
de $0{,}398$ a $0{,}379$, el índice de Calinski-Harabasz cae de $22{,}1$ a
$19{,}6$ y el índice de Davies-Bouldin empeora de $0{,}994$ a $1{,}076$. Las
soluciones con 9, 10 y 11 componentes conservan la misma partición de cuatro
perfiles, pero no mejoran sus métricas internas. Se retienen, por tanto, las 8
componentes por ofrecer el mejor balance entre las tres métricas de validación
interna y la parsimonia de la representación, y no un porcentaje de varianza
explicada fijado de antemano: agregar componentes de casuística más allá de
este punto introduce dimensiones que degradan la cohesión y separación del
clustering sin alterar la partición.

\begin{table}[htbp]
\centering
\caption{Efecto del número de componentes PCA de casuística sobre las métricas de validación interna (Ward $K=4$).}
\label{tab:barrido-k-pca}
\begin{tabular}{c c c c c l}
\toprule
\textbf{$K$ (componentes)} & \textbf{Varianza acum.} & \textbf{Silhouette} & \textbf{CH} & \textbf{DB} & \textbf{Tamaños} \\
\midrule
\bfseries 8 & \bfseries 72{,}3\,\% & \bfseries 0{,}398 & \bfseries 22{,}1 & \bfseries 0{,}994 & 3, 54, 5, 3 \\
9 & 75{,}2\,\% & 0{,}391 & 21{,}1 & 1{,}031 & 3, 54, 5, 3 \\
10 & 77{,}7\,\% & 0{,}383 & 20{,}3 & 1{,}046 & 3, 54, 5, 3 \\
11 & 80{,}0\,\% & 0{,}379 & 19{,}6 & 1{,}076 & 3, 54, 5, 3 \\
\bottomrule
\end{tabular}
\par\smallskip
...
\caption{Tres cargas de mayor magnitud por componente PCA de casuística.}
\label{tab:cargas-pca} \\
\toprule
\textbf{Componente} & \textbf{Carga 1} & \textbf{Carga 2} & \textbf{Carga 3} \\
\midrule
\endfirsthead
\multicolumn{4}{l}{\textit{Tabla~\thetable\ (continuación)}} \\
\toprule
\textbf{Componente} & \textbf{Carga 1} & \textbf{Carga 2} & \textbf{Carga 3} \\
\midrule
\endhead
...
\paragraph{Matriz final de variables}
...
comparable, de modo que sus diferencias de escala se abordan directamente mediante el escalado robusto.
...
\paragraph{Diagnóstico y sensibilidad de bloques de variables}
...
gestión, diversidad y CMA (9 variables); perfil operativo no demográfico (11 variables);
demanda demográfica-obstétrica (7 variables), y componentes PCA de casuística (8 variables). El
...
de cuadrados intra-grupo. Para dos clústeres $A$ y $B$, el criterio es:
...
donde $|A|$ y $|B|$ son los tamaños de los clústeres y $\mathbf{c}_A$,
...
\paragraph{Prueba de permutación sobre el flujo completo}
...
Esta nula por permutación responde específicamente al caso de variables sin
ninguna dependencia conjunta; por ello, funciona como un control de referencia,
pero no conserva las correlaciones presentes en la matriz institucional.

Como control más exigente, se simulan además 500 matrices normales
multivariadas de 65 hospitales con la media y matriz de covarianza estimadas
sobre las 36 variables numéricas transformadas y escaladas. En cada réplica se
aplica Ward sobre las 35 variables de la matriz de clustering y
Kruskal--Wallis con corrección BH sobre las 28 variables interpretables. Esta
nula conserva en esperanza la covarianza lineal observada, pero no preserva las
observaciones originales, las etiquetas hospitalarias ni una estructura de
grupos. Tampoco conserva necesariamente las distribuciones marginales ni las
dependencias no lineales, por lo que se interpreta como complemento de la
permutación por columnas y no como una validación externa. Ambas distribuciones
nulas se reportan en el Capítulo~\ref{cap:resultados}.
...
\paragraph{Discriminación mediante Kruskal-Wallis}
...
\paragraph{Regresión logística con penalización LASSO}
\label{sec:lasso}
...
\paragraph{Comparación directa de homogeneidad interna}
...
\paragraph{Validación cruzada por bloques de variables}
...
interpretación como especialización genuina, perfil híbrido o posible
...
mediante PCA. Esta proyección tiene un propósito exclusivamente ilustrativo y
exploratorio: facilita inspeccionar la disposición relativa de los hospitales y
no constituye evidencia cuantitativa de la validez del clustering ni sustituye
los índices de concordancia de partición (ARI, NMI) que se emplean para ese
propósito en la Sección~\ref{sec:sensibilidad}. Una proyección bidimensional
omite necesariamente información de las 35 variables originales, por ello, su
lectura es cualitativa y queda subordinada a los análisis de sensibilidad y
estabilidad.
...
de diseño, se recalcula el pipeline completo y se compara la partición obtenida con la del pipeline definitivo mediante el índice de
...
\addlinespace
Proyección 2D & PCA & Inspección visual exploratoria; no constituye validación cuantitativa. \\
\bottomrule
\end{tabular}
\end{table}
...
descubriendo automáticamente tanto los clústeres como los hospitales de perfil
...
(Tabla~\ref{tab:anexo-excluidos}) con los datos parciales de uno o dos años que
...
hospitales altamente concentrados en un único GRD, por lo que la robustez
